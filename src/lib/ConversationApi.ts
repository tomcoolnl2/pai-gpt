...
export class ConversationApi {
	 ...
	 * @throws {Error} - Throws an error if the response is not valid.
	 * @private
	 */
		...
		try {
			...
			if (this.responseIsValid(response)) {
				if (response.body) {
					...
					this.handleError('Response body is null');
				}
			}
		} catch (error: unknown) {
	 ...
	 * @throws {Error} - Throws an error if there is an issue with creating the conversation.
	 */
	public async createConversation(payload: MessagePayload): Promise<Conversation | undefined> {
		try {
	 ...
	 * @throws {Error} - Throws an error if there is an issue with loading the conversation.
	 */
	 ...
	 * @param {string} conversationId - The ID of the conversation to delete.
	 * @returns {Promise<boolean>} - A promise that resolves with a boolean indicating deletion success.
	 * @throws {Error} - Throws an error if there is an issue with deleting the conversation.
	 */
	public async deleteConversation(conversationId: string): Promise<boolean | undefined> {
		try {
	 ...
	 * @throws {Error} - Throws an error if there is an issue with retrieving the list of conversations.
	 */
	public async getConversationList(): Promise<Conversation[] | undefined> {
		try {
			...
			if (this.responseIsValid(response)) {
				...
				return data.conversations.map(({ _id, title }: ConversationPayload) => new Conversation(_id, title));
			}
		} catch (error: unknown) {
	 ...
	 * @throws {Error} - Throws an error if there is an issue with adding the message.
	 */
	public async addMessage(conversationId: string, payload: MessagePayload): Promise<boolean | undefined> {
		try {
	 ...
	 * @throws {Error} - Throws an error if the messages are null.
	 * @private
	 */
